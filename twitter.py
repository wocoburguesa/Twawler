...
class Api(object):
  ...
  def GetFollowersOfUser(self, user, cursor=None):
    """Fetch the sequence of twitter.User instances of specified user,
    ...
    Args:
      page:
        Specifies the cursor of results to retrieve.
        For continued operation, cursor should be passed as -1 in
        a loop.
    ...
    if not self._oauth_consumer:
      raise TwitterError("twitter.Api instance must be authenticated")
    user_id = user.AsDict()['screen_name']
    url = '%s/statuses/followers/%s.json' % (self.base_url, user_id)
    parameters = {}
    if cursor:
      parameters['cursor'] = cursor
    else:
    ...
    data = self._ParseAndCheckTwitter(json)
    return ([User.NewFromJsonDict(x) for x in data['users']],
            data['previous_cursor'], data['next_cursor'])
